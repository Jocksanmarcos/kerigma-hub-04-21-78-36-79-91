-- Corrigir schema da Bíblia: remover constraints existentes e alterar para text IDs

-- Primeiro, remover constraints existentes se existirem
DO $$ BEGIN
  -- Remover constraint de versiculos -> livros
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_schema='public' AND table_name='biblia_versiculos' AND constraint_name='biblia_versiculos_livro_id_fkey'
  ) THEN
    ALTER TABLE public.biblia_versiculos DROP CONSTRAINT biblia_versiculos_livro_id_fkey;
  END IF;
  
  -- Remover constraint de versiculos -> versoes
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_schema='public' AND table_name='biblia_versiculos' AND constraint_name='biblia_versiculos_versao_id_fkey'
  ) THEN
    ALTER TABLE public.biblia_versiculos DROP CONSTRAINT biblia_versiculos_versao_id_fkey;
  END IF;
  
  -- Remover constraint de livros -> versoes
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_schema='public' AND table_name='biblia_livros' AND constraint_name='biblia_livros_versao_id_fkey'
  ) THEN
    ALTER TABLE public.biblia_livros DROP CONSTRAINT biblia_livros_versao_id_fkey;
  END IF;
END $$;

-- Criar tabelas com estrutura correta se não existirem
...
-- Alterar tipos de colunas existentes se necessário
DO $$ BEGIN
  -- Alterar biblia_versoes.id para text
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema='public' AND table_name='biblia_versoes' AND column_name='id' AND data_type <> 'text'
  ) THEN
    ALTER TABLE public.biblia_versoes ALTER COLUMN id TYPE text USING id::text;
  END IF;
  
  -- Alterar biblia_livros.id para text
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Alterar biblia_livros.versao_id para text
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Alterar biblia_versiculos.versao_id para text
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Alterar biblia_versiculos.livro_id para text
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Recriar foreign key constraints com os novos tipos
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
