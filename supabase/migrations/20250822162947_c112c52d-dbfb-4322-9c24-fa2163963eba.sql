-- Função para sincronizar todos os vínculos familiares existentes
  ...
  LOOP
    ...
    IF v_pessoa.mae_id IS NOT NULL THEN
      INSERT INTO vinculos_familiares (
        ...
        v_pessoa.id, v_familia_id, 'filho', false, v_pessoa.mae_id
      ) ON CONFLICT (pessoa_id, familia_id, pessoa_relacionada_id) DO NOTHING;
      
      INSERT INTO vinculos_familiares (
        pessoa_id, familia_id, tipo_vinculo, responsavel_familiar, pessoa_relacionada_id
      ) VALUES (
        v_pessoa.mae_id, v_familia_id, 'mae', true, v_pessoa.id
      ) ON CONFLICT (pessoa_id, familia_id, pessoa_relacionada_id) DO NOTHING;
    END IF;
