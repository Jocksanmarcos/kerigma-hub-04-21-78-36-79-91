...
CREATE POLICY "Membros autenticados podem ver relacionamentos" ON public.relacionamentos_familiares FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Admins podem gerenciar relacionamentos" ON public.relacionamentos_familiares FOR ALL USING ((SELECT raw_user_meta_data->>'role' FROM auth.users WHERE id = auth.uid()) = 'administrador');
...
-- Política de segurança EXTREMAMENTE RESTRITA:
