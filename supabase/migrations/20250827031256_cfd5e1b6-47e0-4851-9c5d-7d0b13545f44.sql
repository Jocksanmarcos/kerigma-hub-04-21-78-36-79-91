-- Primeiro vamos verificar a estrutura da tabela permissions
-- E criar as tabelas de conteúdo sem depender da tabela permissions
...
-- Políticas RLS básicas (usando is_admin para simplificar)
CREATE POLICY "Admins podem gerenciar comunicados" ON public.comunicados
  FOR ALL 
  USING (is_admin())
  WITH CHECK (is_admin());
...
CREATE POLICY "Admins podem gerenciar banners" ON public.banners_home
  FOR ALL 
  USING (is_admin())
  WITH CHECK (is_admin());
...
CREATE POLICY "Admins podem gerenciar palavra da semana" ON public.palavra_semana
  FOR ALL 
  USING (is_admin())
  WITH CHECK (is_admin());
